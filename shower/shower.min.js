...
!function(global) {
  var undef, DECL_STATES = {
    ...
    var curOptions = {
      ...
      "string" == typeof modules && (modules = [modules]),
      waitForNextTick || (waitForNextTick = !0,
      nextTick(onNextTick)),
      ...
      for (var name in options)
        options.hasOwnProperty(name) && (curOptions[name] = options[name])
    }
      , getStat = function() {
      var module, res = {};
      for (var name in modulesStorage)
        modulesStorage.hasOwnProperty(name) && (module = modulesStorage[name],
      ...
      for (pendingRequires = []; require = requiresToProcess[i++]; )
      ...
      for (var dep, decl, decls = [], onDeclResolved = function(_, error) {
        if (error)
          return void cb(null, error);
        if (!--unresolvedDepsCnt) {
          for (var decl, exports = [], i = 0; decl = decls[i++]; )
        ...
        "string" == typeof dep) {
          if (!modulesStorage[dep])
            return void cb(null, buildModuleNotFoundError(dep, fromDecl));
      ...
      if (decl.state === DECL_STATES.RESOLVED)
        return void cb(decl.exports);
      if (decl.state === DECL_STATES.IN_RESOLVING)
        return void (curOptions.trackCircularDependencies && isDependenceCircular(decl, path) ? cb(null, buildCircularDependenceError(decl, path)) : decl.dependents.push(cb));
      if (decl.dependents.push(cb),
      decl.prev && !curOptions.allowMultipleDeclarations)
        return void provideError(decl, buildMultipleDeclarationError(decl));
      ...
      requireDeps(decl, deps, path, function(depDeclsExports, error) {
        return error ? void provideError(decl, error) : (depDeclsExports.unshift(function(exports, error) {
          return isProvided ? void cb(null, buildDeclAreadyProvidedError(decl)) : (isProvided = !0,
          void (error ? provideError(decl, error) : provideDecl(decl, exports)))
        }),
        void decl.fn.apply({
          name: decl.name,
          deps: decl.deps,
      ...
      for (var dependent, i = 0; dependent = decl.dependents[i++]; )
        dependent(exports);
    ...
    for (var pathDecl, strPath = [], i = 0; pathDecl = path[i++]; )
      strPath.push(pathDecl.name);
    return strPath.push(decl.name),
    Error('Circular dependence has been detected: "' + strPath.join(" -> ") + '"')
    ...
    if (global.postMessage && !global.opera) {
      ...
      if (isPostMessageAsync) {
        var msg = "__modules" + +new Date
          ...
          e.data === msg && (e.stopPropagation && e.stopPropagation(),
          callFns())
        };
...
shower.modules.define("shower", ["shower.global"], function(provide, showerGlobal) {
  ...
  extend(EventEmitter.prototype, {
    ...
    once: function(eventType, callback, context, priority) {
      var handler = function(event) {
        ...
        context ? callback.call(context, event) : callback(event)
      };
      return this.on(eventType, handler, this, priority),
      this
    },
    emit: function(eventType, eventObject) {
      var event = eventObject
        , listeners = this._listeners;
      event && "function" == typeof event.get || (event = this.createEventObject(eventType, eventObject, this._context)),
      event.isPropagationStopped() || (listeners.hasOwnProperty(eventType) && this._callListeners(listeners[eventType], event),
      this._parent && !event.isPropagationStopped() && this._parent.emit(eventType, event))
    },
    ...
    _removeListener: function(eventType, callback, context, priority) {
      ...
      if (listeners) {
        for (var foundIndex = -1, i = 0, l = listeners.length; i < l; i++)
          listener = listeners[i],
          listener.callback == callback && listener.context == context && listener.priority == priority && (foundIndex = i);
    ...
    _callListeners: function(listeners, event) {
      ...
      for (listeners.sort(sortByPriority); i >= 0 && !event.defaultPrevented(); ) {
        ...
        listener && (listener.context ? listener.callback.call(listener.context, event) : listener.callback(event)),
        i--
      }
    }
  }),
...
shower.modules.define("emitter.Event", ["util.extend"], function(provide, extend) {
  ...
  extend(Event.prototype, {
    ...
    stopPropagation: function() {
      return this._stopPropagation = !0,
...
shower.modules.define("emitter.EventGroup", ["util.extend"], function(provide, extend) {
  ...
  extend(EventGroup.prototype, {
    on: function(types, callback, context) {
      ...
      return this.events.on(types, callback, context),
      this
    },
    ...
    offAll: function() {
      for (var i = 0, k = this._listeners.length; i < k; i += 3)
        this.events.off(this._listeners[i], this._listeners[i + 1], this._listeners[i + 2]);
      return this._listeners.length = 0,
      this
    },
    _removeListener: function(type, callback, context) {
      for (var index = this._listeners.indexOf(type, 0); index != -1; )
        this._listeners[index + 1] == callback && this._listeners[index + 2] == context && (this._listeners.splice(index, 3),
...
shower.modules.define("Plugins", ["Emitter", "util.extend"], function(provide, EventEmitter, extend) {
  ...
  extend(Plugins.prototype, {
    ...
    remove: function(name) {
      ...
      return delete this._plugins[name],
    ...
    _instanceFor: function(shower) {
      for (var name in this._plugins)
        this._plugins.hasOwnProperty(name) && this._instance(this._plugins[name], shower)
    },
...
shower.modules.define("Options", ["Emitter", "options.Monitor", "util.Store", "util.extend", "util.inherit"], function(provide, EventEmitter, Monitor, Store, extend, inherit) {
  ...
  inherit(Options, Store, {
    ...
    unset: function(name) {
      return Options.super.unset(this, name),
...
shower.modules.define("options.Monitor", ["util.extend"], function(provide, extend) {
  function Monitor(options) {
    ...
    this._optionsEvents = options.events.group().on(["set", "unset"], this._onOptionsChange, this),
    this._fieldsHanders = {}
  }
  extend(Monitor.prototype, {
    ...
    add: function(field, callback, context) {
      if (Array.prototype.isArray.call(null, field)) {
        ...
        for (var fieldName in fields)
          fields.hasOwnProperty(fieldName) && this._addHandler(fieldName, callback, context)
      } else
    ...
    remove: function(field, callback, context) {
      if (Array.prototype.isArray.call(null, field)) {
        var fields = field;
        for (var fieldName in fields)
          fields.hasOwnProperty(fieldName) && this._remodeHandler(fieldName, callback, context)
      } else
...
shower.modules.define("Shower", ["Emitter", "Options", "shower.global", "shower.defaultOptions", "shower.Container", "shower.Player", "shower.Location", "shower.slidesParser", "util.extend"], function(provide, EventEmitter, Options, showerGlobal, defaultShowerOptions, Container, Player, Location, defaultSlidesParser, extend) {
  function Shower(container, options) {
    ...
    this.options.get("debug_mode") && (document.body.classList.add(this.options.get("debug_mode_classname")),
    console.info("Debug mode on")),
    this.options.get("hotkeys") || this.disableHotkeys(),
  ...
  extend(Shower.prototype, {
    ...
    remove: function(slide) {
      ...
      if ("number" == typeof slide)
        ...
        if (this._slides.indexOf(slide) == -1)
          throw new Error("Slide not found");
        slidePosition = this._slides.indexOf(slide)
      }
      return slide = this._slides.splice(slidePosition, 1)[0],
    ...
    disableHotkeys: function() {
      return this._isHotkeysOn = !1,
      this
    },
    enableHotkeys: function() {
      return this._isHotkeysOn = !0,
      this
    },
    ...
    updateLiveRegion: function(content) {
      return this._liveRegion.innerHTML = content,
      this
    },
...
shower.modules.define("shower.Container", ["Emitter", "util.bound", "util.extend"], function(provide, EventEmitter, bound, extend) {
  ...
  extend(Container.prototype, {
    init: function() {
      ...
      bodyClassList.contains(listModeClass) || bodyClassList.contains(fullModeClass) || bodyClassList.add(listModeClass),
    ...
    enterSlideMode: function() {
      var bodyClassList = document.body.classList
        , showerOptions = this._shower.options;
      return bodyClassList.remove(showerOptions.get("mode_list_classname")),
    ...
    exitSlideMode: function() {
      var elementClassList = document.body.classList
        , showerOptions = this._shower.options;
      return elementClassList.remove(showerOptions.get("mode_full_classname")),
    ...
    scrollToCurrentSlide: function() {
      ...
      return slideElement && window.scrollTo(0, slideElement.offsetTop),
      this
    },
    _setupListeners: function() {
      this._showerListeners = this._shower.events.group().on("slideadd", this._onSlideAdd, this).on("slideremove", this._onSlideRemove, this),
    ...
    _onKeyDown: function(e) {
      if (this._shower.isHotkeysEnabled())
        ...
        case 116:
          ...
          break;
        case 80:
          !this.isSlideMode() && e.altKey && e.metaKey && (e.preventDefault(),
          this.enterSlideMode())
        }
    }
  }),
...
shower.modules.define("shower.Location", ["util.SessionStore", "util.bound", "util.extend"], function(provide, SessionStore, bound, extend) {
  ...
  extend(Location.prototype, {
    ...
    _init: function() {
      var slideInfo, shower = this._shower, currentSlideId = window.location.hash.substr(1), slideModeClass = shower.options.get("mode_full_classname");
      window.location.hash = "",
      (this.state.get("isSlideMode") || document.body.classList.contains(slideModeClass)) && shower.container.enterSlideMode(),
      "" !== currentSlideId && (slideInfo = this._getSlideById(currentSlideId),
      shower.player.go("undefined" != typeof slideInfo.index ? slideInfo.index : 0))
    },
    _setupListeners: function() {
      var shower = this._shower;
      this._playerListeners = shower.player.events.group().on("activate", this._onSlideActivate, this),
      this._containerListener = shower.container.events.group().on(["slidemodeenter", "slidemodeexit"], this._onContainerSlideModeChange, this),
    ...
    _onPopstate: function() {
      var slideInfo, shower = this._shower, slideId = window.location.hash.substr(1), currentSlide = shower.player.getCurrentSlide(), currentSlideNumber = shower.player.getCurrentSlideIndex();
      this._isSlideMode() && currentSlideNumber === -1 ? shower.player.go(0) : currentSlideNumber === -1 && "" !== window.location.hash && shower.player.go(0),
      currentSlide && slideId !== currentSlide.getId() && (slideInfo = this._getSlideById(slideId),
    ...
    _setTitle: function() {
      ...
      if (isSlideMode && currentSlide) {
        ...
        this._documentTitle !== title && (document.title = this._documentTitle)
    }
  }),
  provide(Location)
}),
shower.modules.define("shower.Player", ["Emitter", "util.bound", "util.extend"], function(provide, EventEmitter, bound, extend) {
  ...
  extend(Player.prototype, {
    init: function() {
      this._showerListeners = this._shower.events.group().on("slideadd", this._onSlideAdd, this).on("slideremove", this._onSlideRemove, this).on("slidemodeenter", this._onSlideModeEnter, this),
      this._playerListeners = this.events.group().on("prev", this._onPrev, this).on("next", this._onNext, this).on("prevslide", this._onPrev, this).on("nextslide", this._onNext, this),
      document.addEventListener("keydown", bound(this, "_onKeyDown"))
    },
    destroy: function() {
      this._showerListeners.offAll(),
    ...
    next: function() {
      return this.events.emit("next"),
      this
    },
    prev: function() {
      return this.events.emit("prev"),
      this
    },
    nextSlide: function() {
      return this.events.emit("nextslide"),
      this
    },
    prevSlide: function() {
      return this.events.emit("prevslide"),
      this
    },
    first: function() {
      return this.go(0),
    ...
    last: function() {
      return this.go(this._shower.getSlidesCount() - 1),
      this
    },
    go: function(index) {
      ...
      return index != this._currentSlideNumber && index < slidesCount && index >= 0 && (currentSlide && currentSlide.isActive() && currentSlide.deactivate(),
      currentSlide = this._shower.get(index),
      this._currentSlide = currentSlide,
      this._currentSlideNumber = index,
      currentSlide.isActive() || currentSlide.activate(),
    ...
    _onKeyDown: function(e) {
      if (this._shower.isHotkeysEnabled() && !/^(?:button|input|select|textarea)$/i.test(e.target.tagName)) {
        ...
        !action || !allowModifiers && (e.altKey || e.ctrlKey || e.metaKey) || (e.preventDefault(),
        this[action]())
      }
    },
...
shower.modules.define("shower.slidesParser", ["Slide"], function(provide, Slide) {
  function parse(containerElement, cssSelector) {
    ...
    return slidesElements = Array.prototype.slice.call(slidesElements),
    slidesElements.map(function(slideElement, index) {
      var slide = new Slide(slideElement);
      return slideElement.id || (slideElement.id = index + 1),
      slide
    })
  }
  provide(parse)
...
shower.modules.define("Slide", ["shower.defaultOptions", "Emitter", "Options", "slide.Layout", "slide.layoutFactory", "util.Store", "util.extend"], function(provide, defaultOptions, EventEmitter, OptionsManager, Layout, slideLayoutFactory, DataStore, extend) {
  ...
  extend(Slide.prototype, {
    ...
    activate: function() {
      ...
      return this.state.set("visited", ++visited),
      this.events.emit("activate", {
        slide: this
      }),
      this
...
shower.modules.define("slide.Layout", ["Options", "shower.defaultOptions", "Emitter", "util.bound", "util.extend"], function(provide, OptionsManager, defaultOptions, EventEmitter, bound, extend) {
  ...
  extend(Layout.prototype, {
    ...
    setParent: function(parent) {
      this._parent != parent && (this._clearListeners(),
      this._parent = parent,
      this._parent && this._setupListeners(),
    ...
    setParentElement: function(parentElement) {
      parentElement != this._parentElement && (this._parentElement = parentElement,
    ...
    setTitle: function(title) {
      ...
      titleElement ? titleElement.innerHTML = title : (titleElement = document.createElement(titleElementSelector),
    ...
    _setupListeners: function() {
      this._slideListeners = this._parent.events.group().on("activate", this._onSlideActivate, this).on("deactivate", this._onSlideDeactivate, this),
      this._element.addEventListener("click", bound(this, "_onSlideClick"), !1)
    },
    _clearListeners: function() {
      this._slideListeners && this._slideListeners.offAll(),
...
shower.modules.define("slide.layoutFactory", ["slide.Layout", "util.extend"], function(provide, SlideLayout, extend) {
  ...
  extend(layoutFactory, {
    ...
    _createElement: function(options) {
      ...
      return element.innerHTML = options.content,
  ...
  function polyfill(target) {
    for (var i = 1, l = arguments.length; i < l; i++) {
      ...
      for (var property in obj)
        obj.hasOwnProperty(property) && (target[property] = obj[property])
    }
    return target
  }
  provide(extend)
...
shower.modules.define("util.inherit", ["util.extend"], function(provide, extend) {
  var inherit = function(childClass, parentClass, override) {
    return childClass.prototype = Object.create(parentClass.prototype),
    childClass.prototype.constructor = childClass,
    childClass.super = parentClass.prototype,
    childClass.super.constructor = parentClass,
    override && extend(childClass.prototype, override),
...
shower.modules.define("shower-next", ["shower", "Emitter", "util.extend"], function(provide, globalShower, EventEmitter, extend) {
  function Next(shower, options) {
    ...
    this._setupListeners(),
    this._shower.player.getCurrentSlideIndex() != -1 && this._onSlideActivate()
  }
  ...
  extend(Next.prototype, {
    ...
    getLength: function() {
      return this._elements = this._getElements(),
    ...
    _setupListeners: function() {
      var shower = this._shower;
      this._showerListeners = shower.events.group().on("destroy", this.destroy, this),
      this._playerListeners = shower.player.events.group().on("activate", this._onSlideActivate, this).on("next", this._onNext, this).on("prev", this._onPrev, this);
      ...
      timerPlugin ? this._setupTimerPluginListener(timerPlugin) : this._pluginsListeners = globalShower.plugins.events.group().on("add", function(e) {
        e.get("name") === TIMER_PLUGIN_NAME && (this._setupTimerPluginListener(),
    ...
    _clearListeners: function() {
      this._showerListeners.offAll(),
      this._playerListeners.offAll(),
      this._pluginsListeners && this._pluginsListeners.offAll()
    },
    ...
    _onNext: function(e) {
      ...
      isSlideMode && elementsLength && this._innerComplete < elementsLength && (e.preventDefault(),
      this.next())
    },
    _onPrev: function(e) {
      var elementsLength = this._elements.length
        , completed = (this._shower.container.isSlideMode(),
      this._innerComplete);
      elementsLength && completed < elementsLength && completed > 0 && (e.preventDefault(),
      this.prev())
    },
    _go: function() {
...
shower.modules.define("shower-progress", ["util.extend"], function(provide, extend) {
  function Progress(shower, options) {
    ...
    this._element && (this._setupListeners(),
  ...
  extend(Progress.prototype, {
    ...
    updateProgress: function() {
      ...
      this._element && (this._element.style.width = currentProgressValue.toFixed(2) + "%",
    ...
    _setupListeners: function() {
      var shower = this._shower;
      this._showerListeners = shower.events.group().on("destroy", this.destroy, this),
      this._playerListeners = shower.player.events.group().on("activate", this._onSlideChange, this)
    },
    _clearListeners: function() {
      this._showerListeners && this._showerListeners.offAll(),
      this._playerListeners && this._playerListeners.offAll()
    },
...
shower.modules.define("shower-timer", ["shower", "Emitter", "util.extend"], function(provide, showerGlobal, EventEmitter, extend) {
  ...
  extend(Timer.prototype, {
    ...
    _setupListeners: function() {
      ...
      this._showerListeners = shower.events.group().on("destroy", this.destroy, this),
      this._playerListeners = shower.player.events.group().on("keydown", this._clearTimer, this).on("activate", this._onSlideActivate, this),
      this._nextPlugin = showerGlobal.plugins.get(PLUGIN_NAME_NEXT, shower),
      this._nextPlugin || (this._pluginsListeners = shower.plugins.events.group().on("pluginadd", function(e) {
        e.get("name") === PLUGIN_NAME_NEXT && (this._nextPlugin = shower.plugins.get(PLUGIN_NAME_NEXT),
        this._pluginsListeners.offAll())
      }, this)),
      shower.player.getCurrentSlideIndex() != -1 && this._onSlideActivate()
    },
    _clearListeners: function() {
      this._showerListeners.offAll(),
      this._playerListeners.offAll()
    ...
    _onSlideActivate: function() {
      ...
      if (this._shower.container.isSlideMode() && currentSlide.state.get("visited") < 2) {
        ...
        timing && /^(\d{1,2}:)?\d{1,3}$/.test(timing) && (timing.indexOf(":") !== -1 ? (timing = timing.split(":"),
        timing = 1e3 * (60 * parseInt(timing[0], 10) + parseInt(timing[1], 10))) : timing = 1e3 * parseInt(timing, 10),
        0 !== timing && this._initTimer(timing))
      }
    },
    _initTimer: function(timing) {
      var events = this.events
        , nextPlugin = (this._shower,
      this._nextPlugin);
      nextPlugin && nextPlugin.getLength() && nextPlugin.getLength() != nextPlugin.getComplete() && (timing /= nextPlugin.getLength() + 1),
...
shower.modules.define("shower-touch", ["util.extend"], function(provide, extend) {
  ...
  extend(Touch.prototype, {
    ...
    _setupListeners: function() {
      var shower = this._shower;
      this._showerListeners = shower.events.group().on("add", this._onSlideAdd, this),
      ...
      document.addEventListener("touchmove", this._bindedTouchMove, !0)
    },
    _clearListeners: function() {
      this._showerListeners.offAll(),
      this._shower.getSlides().forEach(this._removeTouchStartListener, this),
      document.removeEventListener("touchmove", this._bindedTouchMove, !1)
    },
    ...
    _onTouchStart: function(e) {
      var x, shower = this._shower, isSlideMode = shower.container.isSlideMode(), element = e.target, slide = this._getSlideByElement(e.currentTarget);
      slide && (isSlideMode && !this._isInteractiveElement(element) && (e.preventDefault(),
      x = e.touches[0].pageX,
      x > window.innerWidth / 2 ? shower.player.next() : shower.player.prev()),
      isSlideMode || slide.activate())
    },
